...
Tmb_Bappenas=Img2Thumbz(LogBapenasz70,270,91,0.75),Tmb_KMtn=Img2Thumbz(Log_Kmentan,128,129),
Tmb_Lpn_WRI = Img2Thumbz(Log_BRIN8,125,90,0.7) ;
//print('Logo',Tmb_Log8); Tmb_BRIN,Tmb_Log8,Tmb_Bappenas
//MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM
var S1_Indo = Radar.S1_Indo(); 
var Point,SH_Sri_Kord=[107.6352944,-6.31700999], SH_Sri_Point = Kords2Geom(SH_Sri_Kord,'Point');
var Kord_Brigif=[106.862734,-6.3379290],Lap_Brigif_Point=Kords2Geom(Kord_Brigif,'Point'),Kord_Point;
var Point_Gab_Nam = ['Lap_Brigif','SH_Sri','Sawit Asahan','Sawit Tandun','DI Rentang','Admin Prov','Current Bound']
...
LstTgl_L8,LstTgl_L8d,LstImgId_L8,LstImgId_L8d,Lbl_Tgl_L8,Arr_NamPrv,NamPrv,Prov_Bound
; 
...
var Pil_S2 = ui.Select({placeholder : ' S2 Data '}),Pil_S1 = ui.Select({placeholder : ' S1 Data '}),
Pil_L8 = ui.Select({placeholder : ' L8 Data '}),
...
Tgl_R = [TglS,TglE] ; Point = SH_Sri_Point; Main(SH_Sri_Point,17); 
Set_Map(9,FTxt.Feat2Img(Indo_Kc,1,1,''),{palette:'black'},'Sub District');
// Set_Map(10,FTxt.Feat2Img(SH_Sri_Poly,0,1,'ID'),Vis_Sri,'Polygon Sawah SH Sri,Subang',0);
// Set_Map(11,FTxt.Feat2Img(SH_Sri_Poly,1,2,''),{palette:'880088'},'Sawah SH Sri,Subang');
// Set_Map(12,Jln_Indo,{color:'red'},'Street'); 
Set_Map(13,Grid_Indo_1,{palette:'black'},'Grid 1 Deg');
...
Pil_S1 = ui.Select({placeholder : ' S1 Data ', items : LstTgl_S1d});  
...
Panel_1.style().set({width:'300px'});
Panel_2.style().set({width:'300px',height:'300px'});
...
/*
Panel_1.add(Lbl_Judul); Panel_1.add(Lbl_Inv); Panel_1.add(Lbl_Afil); // Panel_1.add(Lbl_Point);
Panel_1.add(TB_Point); Panel_1.add(Lbl_TglS); Panel_1.add(TB_SD); 
Panel_1.add(Btn_Repro); 
//Panel_1.add(Lbl_TglE);Panel_1.add(TB_ED); 
Panel_1.add(Pil_S2); Panel_1.add(Pil_L8);  Panel_1.add(Pil_S1);
Panel_2.add(Lbl_Inv2); Panel_2.add(Lbl_Inv2); 
*/
//Map.add(Pil_S2);   
...
Pil_Prv.onChange(function(Cek){
 ...
 Set_Map(8, Poly2Img(Point,1,3,0),{palette:'aa0000'},'OutLine Prov ' + NamPrv);
 Prov_Bound = Obj2Bound (Point) //Point.geometry().bounds() );
}); 
...
Btn_Repro.onClick(function(){
  ...
  Kord_Point= [Xt,Yt] ; Point = ee.Geometry.Point(Kord_Point);
...
//Pil_S2 = ui.Select({placeholder : ' S2 Data ',items:LstTgl_S2d}); 
//Pil_L8 = ui.Select({placeholder : ' L8 Data ',items:LstTgl_L8d});
  Map.setCenter(Xt,Yt,17); Set_Pilih(Point);
});
}
...
Set_Map(1,L8.map(CMsk.CMsk_L8b).median().select(CMsk.Bnd_Optis()),VisRGB_NCC_Mos,'RGB NCC L8 Mosaik ' +  TglS_L8 + ' to ' + TglE_L8);
Set_Map(2,S2.map(CMsk.CMsk_S2K).median().select(CMsk.Bnd_Optis()),VisRGB_NCC_Mos,'RGB NCC S2 Mosaik ' + TglS_S2 + ' to ' + TglE_S2);
...
Set_Map(7,Point.buffer(56),{color:'cyan'},'Buffer 1 Ha',1,0.5);
Set_Map(8,Point,{color:'red'},'Point Interest');
Lbl_Tgl_S1.setValue('S1 ' + LstTgl_S1[JumLst_S1-1] );
Lbl_Tgl_S2.setValue('S2 ' + LstTgl_S2[JumLst_S2-1] ); 
Lbl_Tgl_L8.setValue('L8 ' + LstTgl_L8[JumLst_L8-1] );
return 1;}
